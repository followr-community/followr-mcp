...
RELATED TOOLS IN THE PLANNING WORKFLOW (named explicitly so the host's tool-search precaches them alongside this one): deep_research, confirm_industry, detect_brand_visual_style, propose_visual_style_options, confirm_visual_style, draft_content_plan, set_copy_drafts, preview_plan_item, update_content_plan, execute_content_plan. The typical chain when starting from scratch is: prepare_content_plan_context -> deep_research -> confirm_industry -> detect_brand_visual_style (or propose_visual_style_options) -> confirm_visual_style -> draft_content_plan -> set_copy_drafts -> preview_plan_item -> execute_content_plan.`,
      inputSchema: {
        company: z
